...
def measure_and_save_contours(front_image_path, side_image_path, user_height_cm, output_dir="C:\coding\mannequin\old\dimensiongen\images"):
    ...
    cv2.drawContours(front_image, [front_contour], -1, (0, 255, 0), 3)
    cv2.drawContours(side_image, [side_contour], -1, (0, 255, 0), 3)

    # Save or display images with contours
    front_output_path = f"{output_dir}/front_view_with_contours.jpg"
    side_output_path = f"{output_dir}/side_view_with_contours.jpg"
    
    # Save images locally
    cv2.imwrite(front_output_path, front_image)
    cv2.imwrite(side_output_path, side_image)

    print(f"Front view with contours saved to: {front_output_path}")
    print(f"Side view with contours saved to: {side_output_path}")

    # Optionally display images for verification
    cv2.imshow("Front View with Contours", front_image)
    cv2.imshow("Side View with Contours", side_image)
...
measurements = measure_and_save_contours(front_img_path, side_img_path, user_height_cm)
print(measurements)